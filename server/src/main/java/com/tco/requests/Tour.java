...
public abstract class Tour {
    ...
    public Places shorter (Places places, double earthRadius, double response) {
        ...
        for (int i = 0; i < places_arr.length; i++) {
            ...
            // }
            System.out.println("currentTour.length in shorter: " + currentTour.length);
    ...
    private Place[] construct (int startIndex, Place[] places_arr, double earthRadius) {
        Place[] tour = new Place[places_arr.length + 1];
        ...
        int increment = 0;
        while (tour.length < places_arr.length) {
            ...
            tour[increment++] = places_arr[nearestCityIndex];
    ...
    public long calculateTourDistance(Place[] tour, double earthRadius) {
        ...
        for (int i = 0; i < tour.length; i++) {
            Place from = tour[i];
            System.out.println("tour.length in calculate: " + tour.length);
            Place to = (i + 1 < tour.length) ? tour[i + 1] : tour[0]; // loop back to the start
